...
你也可以通过 `kubectl describe` 获取有关该 DataLoad 的更多详细信息：
...
上述 DataLoad 在进行数据加载时，对于 `/spark/spark-2.4.8` 目录下的文件仅会在分布式缓存引擎中保留**1份**数据缓存副本，而对于文件 `/spark/spark-3.0.1/pyspark-3.0.1.tar.gz`
，分布式缓存引擎将会保留**2份**缓存副本。
